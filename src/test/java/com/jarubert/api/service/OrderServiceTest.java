...
public class OrderServiceTest {
    ...
    public void shouldReturnPlacedOrder() {
        ...
        Product p1 = productRepository.save(new Product("Pipe", "Wood pipe", 150.0));
    ...
    private Order createOrder() {
        Product p1 = productRepository.save(new Product("Pipe", "Wood pipe", 150.0));
        Product p2 = productRepository.save(new Product("Grass", "Smokable grass", 17.0));
        Order order = orderRepository.save(new Order(LocalDateTime.now().plus(1 , ChronoUnit.DAYS), "teste@teste.com", Status.APPROVED));
        order.getOrderEntries().add(orderEntryRepository.save(new OrderEntry(order, 1, p1, 10, p1.getPrice())));
        order.getOrderEntries().add(orderEntryRepository.save(new OrderEntry(order, 1, p2, 10, p1.getPrice())));
    ...
    private void compareEntry(OrderEntryPostDto expectedEntry, OrderEntryDto responseEntry) {
        Assert.assertEquals(expectedEntry.getProductId(), responseEntry.getProduct().getId());
    ...
    private void compareProduct(OrderEntryProductDto expectedProduct, OrderEntryProductDto productResponse) {
